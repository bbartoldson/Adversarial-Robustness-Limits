...
def trades_loss(model, x_natural, y, optimizer, step_size=0.003, epsilon=0.031, perturb_steps=10, beta=1.0, 
                attack='linf-pgd', device=None):
    """
    TRADES training (Zhang et al, 2019).
    """
    # unnecessary if grads on delta do not sync. TODO: check if they do. https://pytorch.org/docs/stable/generated/torch.nn.parallel.DistributedDataParallel.html?highlight=no_sync#torch.nn.parallel.DistributedDataParallel.no_sync
    context = model.no_sync() if hasattr(model,'no_sync') else suppress() 
...
def trades_loss_LSE(model, x_natural, y, optimizer, step_size=0.003, epsilon=0.031, perturb_steps=10, beta=1.0, 
                attack='linf-pgd', label_smoothing=0.1, device=None):
    """
    SCORE training (Ours).
    """
